...
def get_rng(seed: Optional[int]) -> Generator:
    """Return a numpy Generator seeded deterministically (or system-random if seed is None)."""
    return default_rng(int(seed) if seed is not None else None)

def generate_bits_and_bases(n: int, rng: Optional[Generator] = None):
    """Generate random bits and bases using provided numpy Generator (default_rng)."""
    if rng is None:
        rng = default_rng()
...
def eavesdrop(qubits, alice_bits, alice_bases, eve_prob=0.3, sim_backend=None, seed: Optional[Generator] = None):
    """
    ...
    # Use numpy Generator; if seed is None → system random each run
    ...
    for i, qc in enumerate(qubits):
        ...
        if rng.random() < eve_prob:
            intercepted=True
            measure_basis = int(rng.integers(0, 2))
...
#A Sampler is a type of quantum computing primitive or program that runs a quantum circuit and returns the outcomes of its measurements. Instead of giving you a single result, it provides a statistical distribution—the probabilities or counts of the possible bitstrings (sequences of 0s and 1s) that can be measured from the circuit.
...
def _extract_single_bit_from_sampler_result(item) -> int:
    ...
    for attr in ("quasi_dists", "quasi_dist", "meas", "c", "counts"):
        ...
        if isinstance(obj, list) and obj and isinstance(obj[0], dict):
            ...
            try:
                ...
                return 0  # fallback to 0 on parsing errors
    ...
    raise RuntimeError(f"Unable to parse Sampler result; schema={dir(data)}")
...
'''Step 1: Divide and Check: Alice and Bob publicly agree to divide their keys into smaller blocks. For each block, they both calculate its parity.

Step 2: Compare: Alice tells Bob her parity for the block.

Step 3: Find the Error:

If their parities match, they know that block is either error-free or has an even number of errors (unlikely if the error rate is low). They move on.

If their parities don't match, they know there's an odd number of errors in that block. They then use a clever method, like the binary search in _binary_search_fix, to pinpoint the exact location of a single error without revealing the bits themselves. They repeatedly check the parity of smaller and smaller halves of the block until they find the single bit that's wrong.

Step 4: Fix It: Once the bad bit is found, Bob flips it in his key. Now their blocks are identical.'''
def cascade_lite(
    alice: List[int],
    bob: List[int],
    block_size: int = 16,
...
def run_bb84(
    ...
    eve_prob : float
        Probability of Eve intercepting a qubit (intercept-resend attack).
    run_on_real_device : bool
        If True, try running on a real IBM quantum device (via Sampler).
    run_on_aer : bool
        If True, try running on AerSimulator (if Qiskit installed).
    run_on_custom : bool
        Reserved for future custom backends.
    noise : bool
        If True, apply channel + readout noise (math path) or Qiskit noise model (Aer path).
    ec : bool
        Enable Cascade-lite error correction.
    pa : bool
        Enable privacy amplification (BLAKE2s).
    pa_target_len : Optional[int]
        Target length of final key after PA (bits).
    seed : Optional[int]
        RNG seed.
    save_keys : bool
        If True, return full arrays (bits, bases, outcomes) for visualization.
    alice_bits_user : Optional[List[int]]
        Pre-specified Alice bits (else random).
    alice_bases_user : Optional[List[int]]
        Pre-specified Alice bases (else random).
    shots_on_device : int
        Number of shots for real-device or Sampler execution.
    sampler : Optional[Sampler]
        External Sampler object (frontend can inject).
    aer_dep : float
        Depolarizing error rate for Aer noise model.
    aer_ro : float
        Readout error rate for Aer noise model.
    distance_km : Optional[float]
        Distance for scaling readout noise (linear model).
    channel_noise_rate : float
        Probability of a bit flip in transit (applied before Bob measures).
        Distinct from readout noise:
        - Channel noise models **physical qubit disturbance in flight**.
        - Readout noise (effective_ro) models **detector misreporting after measurement**.
        Note: if both `channel_noise_rate` and `effective_ro` are nonzero,
        both sources of error apply (they are independent).
    """

    vis_outcomes = None  # initialize for later guard
    rng = get_rng(seed)
    ...
    if distance_km is not None:
        effective_ro = min(1.0, aer_ro + (distance_km or 0) * channel_noise_rate)
    ...
    if eve_prob > 0.0:
        if qubits is not None and HAS_QISKIT:
            qubits, eve_indices, eve_measured_bits, eve_measured_bases = eavesdrop(
                qubits, alice_bits, alice_bases, eve_prob=eve_prob, sim_backend=AerSimulator(), seed=seed
            )
        else:
            rng = np.random.default_rng(seed)
    ...
    # 1) Try real device first (if requested and Qiskit available)
    if run_on_real_device and HAS_QISKIT and qubits is not None:
        try:
            ...
            # fall through to Aer fallback below
            fallback_to_simulator = True
            fallback_message = str(e)

    # 2) If we haven't got results yet, try Aer (if requested and available)
    if bob_results is None and run_on_aer:
        if HAS_QISKIT and qubits is not None:
            try:
                ...
                fallback_to_simulator = True
                fallback_message = str(e)
                bob_results = None  # make sure we will fallback to math below
        else:
            # classical fallback for Aer requested but qiskit not installed
            bob_results = None

    # 3) If still no bob_results, run the classical math-based simulation (single code path)
    if bob_results is None:
        # Use a Generator-based RNG here for sampling probabilities
        rng = np.random.default_rng(seed)

        # Do NOT reset eve telemetry here — it may have been produced by eavesdrop() above.
        # Only append to eve_indices/... if we detect an interception inside this math sim.
        ...
        for i in range(n_qubits):
            ...
            state = prepare_state(int(alice_bits[i]), int(alice_bases[i]))

            # Eve: intercept-resend (measure in random basis, then re-prepare)
            intercepted = False
            if rng.random() < float(eve_prob):
                intercepted = True
                e_basis = int(rng.integers(0, 2))
                e_bit = measure_state(state, e_basis, rng)
                state = prepare_state(int(e_bit), int(e_basis))

                # record Eve telemetry (only append new interceptions)
                if i not in eve_indices:
                    eve_indices.append(i)
                    eve_measured_bases.append(e_basis)
                    eve_measured_bits.append(int(e_bit))

            # Optional channel noise: flip qubit *before* Bob measures
            if noise and (rng.random() < float(channel_noise_rate)):
                state = apply_pauli_x(state)

            # Bob measures in his basis
            b_bit = measure_state(state, int(bob_bases[i]), rng)

            # Optional readout noise: flip measurement with prob effective_ro
            if noise and (rng.random() < float(effective_ro)):
                b_bit ^= 1

            bob_results.append(int(b_bit))

            # Build the visual outcome for this index
            if intercepted:
                resent_bit = int(eve_measured_bits[eve_indices.index(i)])
                resent_basis = int(eve_measured_bases[eve_indices.index(i)])
            else:
                resent_bit = int(alice_bits[i])
                resent_basis = int(alice_bases[i])

            vis_outcomes.append({
                "resent_bit": int(resent_bit),
                "resent_basis": int(resent_basis),
                "bob_bit": int(b_bit),
                "kept": bool(int(bob_bases[i]) == int(alice_bases[i])),
                "error": bool(int(b_bit) != int(alice_bits[i])) and (int(bob_bases[i]) == int(alice_bases[i])),
            })

    # 4) If bob_results was produced by Aer/real device above, ensure vis_outcomes exists for that path
    if bob_results is not None and (vis_outcomes is None or len(vis_outcomes) == 0):
        vis_outcomes = []
        for i in range(n_qubits):
            resent_bit = int(alice_bits[i])
            resent_basis = int(alice_bases[i])
            # If Eve telemetry was recorded earlier (eavesdrop()), use that instead for resent values
            if i in eve_indices:
                idx = eve_indices.index(i)
                resent_bit = int(eve_measured_bits[idx])
                resent_basis = int(eve_measured_bases[idx])
            vis_outcomes.append({
                "resent_bit": int(resent_bit),
                "resent_basis": int(resent_basis),
                "bob_bit": int(bob_results[i]) if i < len(bob_results) else 0,
                "kept": int(int(bob_bases[i]) == int(alice_bases[i])),
                "error": (int(bob_results[i]) != int(alice_bits[i]) if i < len(bob_results) else False) and (int(bob_bases[i]) == int(alice_bases[i])),
            })
    ...
    if len(bob_results) != n_qubits:
        bob_results = (bob_results + [0] * n_qubits)[:n_qubits]
    # --- Sifting ---
    ...
    if pa:
        ...
        if pa_target_len is None:
            min_pa_len = 32  # can be a parameter
            pa_target_len = max(min_pa_len, len(source_key) // 2)
...
def main():
    ...
    parser.add_argument("--aer", action="store_true", help="Run on AerSimulator if available")
    ...
    res = run_bb84(
        ...
        run_on_aer=args.aer,   # explicitly force NOT Aer
        run_on_custom=False,
        noise=args.noise,
        ec=args.ec,
        pa=args.pa,
