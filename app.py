...
# optional Qiskit runtime (guarded so app runs without Qiskit)
try:
...
st.caption("Explore the fascinating world of quantum key distribution with interactive simulations.")
...
with st.sidebar:
    ...
    preset = st.selectbox("Scenario Presets", ["Custom (manual)", "Clean Fiber (demo)", "Noisy Fiber", "Aggressive Eve", "Hardware Demo"])
...
use_custom_sim = backend_choice == "Custom Classical Simulator"
use_aer = backend_choice == "Aer Simulator"
use_real_device = backend_choice == "Real Quantum Device"
# keep session-state in sync with selection so later code can read it
# prefer explicit session_state if present; otherwise use selection
use_real_device = bool(st.session_state.get("use_real_device", use_real_device))
...
if use_real_device:
    try:
        ...
        if not real_devices:
            ...
            st.session_state.shots_on_device = st.sidebar.number_input(
                "Hardware shots (real device)",
                min_value=1, max_value=128, value=defaults["shots"], step=1
            )
            ...
            if not HAS_QISKIT_RUNTIME:
                st.sidebar.error("Qiskit runtime not available in this environment — cannot use real device.")
                use_real_device = False
                st.session_state.use_real_device = False
            else:
                try:
                    sampler = Sampler(backend=backend)  # instantiate once here
                    # store sampler in session_state to reuse in run block (optional)
...
if run_clicked:
    ...
    user_bases = None
    if use_custom_sim:
        st.info("Running on classical math simulator (no Aer).")
    elif use_aer:
        st.info("Running on Qiskit Aer simulator.")
    elif use_real_device:
        st.info("Running on real IBM Quantum hardware.")
    ...
    channel_noise_rate = (vis_distance * channel_noise_per_km) / 100.0 if add_noise else 0.0
    with st.spinner("Running BB84 backend..."):
        ...
        if not result or "alice_bits_raw" not in result:
            st.error("Simulation failed. Check backend configuration.")
            st.stop()
    ...
    bob_results = result.get("bob_results", [])
    ...
    eve_indices = [i + 1 for i in result.get("eve_indices", [])]

    eve_bases = result.get("eve_measured_bases", [])
    ...
    # derive visuals from backend output where possible; otherwise synthesize a safe fallback
    vis_outcomes = result.get("vis_outcomes", None)
    # if backend didn't provide vis_outcomes or it's too short, synthesize them
    if not vis_outcomes or len(vis_outcomes) < len(alice_bits):
        vis_outcomes = []
        for i in range(len(alice_bits)):
            intercepted = i in (result.get("eve_indices", []) or [])
            # synthesize a minimal outcome consistent with backend arrays
            resent_bit = (result.get("eve_measured_bits", [])[result["eve_indices"].index(i)]
                        if intercepted and "eve_measured_bits" in result and i in result.get("eve_indices", [])
                        else int(alice_bits[i]))
            resent_basis = (result.get("eve_measured_bases", [])[result["eve_indices"].index(i)]
                            if intercepted and "eve_measured_bases" in result and i in result.get("eve_indices", [])
                            else int(alice_bases[i]))
            # simple bob result fallback: use backend bob_results if present else compute
            bob_bit = result.get("bob_results", [None]*len(alice_bits))[i] if result.get("bob_results") else None
            if bob_bit is None:
                # emulate Bob measurement for fallback visuals:
                if int(bob_bases[i]) == int(resent_basis):
                    bob_bit = int(resent_bit)
                else:
                    bob_bit = int(rng.integers(0,2))
                # local noise simulation for fallback
                if add_noise and rng.random() < ((vis_distance * channel_noise_per_km) / 100.0):
                    bob_bit ^= 1
            kept = int(bob_bases[i]) == int(alice_bases[i])
            error = kept and (int(bob_bit) != int(alice_bits[i]))
            vis_outcomes.append({
                ...
                "kept": bool(kept),
                "error": bool(error),
                "eve_basis": int(resent_basis) if intercepted else None,
                "eve_bit": int(resent_bit) if intercepted else None
            })

    # build e_mask from eve_indices (fallback to False if not present)
    e_mask = np.zeros(len(alice_bits), dtype=bool)
    for idx in (result.get("eve_indices", []) or []):
        if 0 <= idx < len(alice_bits):
            e_mask[idx] = True
    ...
    with col_info:
        ...
        for i in range(min(12, len(alice_bits))):
            ...
            BASIS_DISPLAY = {0: "Z", 1: "X"}
            rows.append({
                "#": i+1,
                "A bit|basis": f"{alice_bits[i]}|{BASIS_DISPLAY[int(alice_bases[i])] }",
                "Eve?": "Yes" if e_mask[i] else "No",
                "Resent": f"{o['resent_bit']}|{BASIS_SYMBOL[int(o['resent_basis'])]}",
                "Bob basis": BASIS_DISPLAY[int(bob_bases[i])],
                "Bob bit": o['bob_bit'],
                "Kept": "✓" if o['kept'] else "–",
    ...
    with st.expander("Keys & Metrics", expanded=True):
        ...
        with c1:
            ...
            st.markdown("".join([f"<span class='keychip'>{BASIS_DISPLAY[b]}</span>" for b in alice_bases[:64]]), unsafe_allow_html=True)
        with c2:
            st.markdown("**Bob (Receiver)**")
            st.write("Bases (0=Z,1=X):")
            st.markdown("".join([f"<span class='keychip'>{BASIS_DISPLAY[b]}</span>" for b in bob_bases[:64]]), unsafe_allow_html=True)
    ...
    def simulate_qber_distance(n_qubits, max_distance, eve_prob, runs=3, channel_noise_per_km=0.002):
        ...
        for d in distances:
            ...
            for _ in range(runs):
                res_no = run_bb84(
                    n_qubits=n_qubits, eve_prob=0.0, noise=add_noise,
                    ec=error_correction, pa=privacy_amplification, seed=seed
                )
                res_e = run_bb84(
                    n_qubits=n_qubits, eve_prob=eve_prob, noise=add_noise,
                    ec=error_correction, pa=privacy_amplification, seed=seed
                )
            ...
            qber_no_eve.append(float(np.mean(r_no)))
            qber_with_eve.append(float(np.mean(r_e)))
        return distances, qber_no_eve, qber_with_eve
    distances, qber_no_eve, qber_with_eve = simulate_qber_distance(
    n_qubits=effective_n, max_distance=200, eve_prob=eve_prob,
    runs=3, channel_noise_per_km=channel_noise_per_km/100.0
    )
    qber_dist_fig = go.Figure()
    qber_dist_fig.add_trace(go.Scatter(x=distances, y=qber_no_eve, mode="lines+markers", name="No Eve"))
    qber_dist_fig.add_trace(go.Scatter(x=distances, y=qber_with_eve, mode="lines+markers", name="With Eve"))
    qber_dist_fig.update_layout(
    ...
    st.plotly_chart(qber_dist_fig, use_container_width=True)
    # -------------------------
    ...
    if teaching_mode and eve_indices:
        with st.expander("Eve Interception Highlights", expanded=True):
            if eve_indices:
                st.markdown(f"Eve intercepted {len(eve_indices)} qubits ({len(eve_indices)/len(alice_bits)*100:.2f}%)")
                eve_indices_display = [i+1 for i in eve_indices] 
                # Intercepted qubit indices
                st.write("**Intercepted qubit indices:**")
                st.markdown(" ".join([f"<span class='keychip'>{i}</span>" for i in eve_indices]), unsafe_allow_html=True)
                
                # Eve bases
                st.write("**Eve bases:**")
                st.markdown(" ".join([f"<span class='keychip'>{BASIS_SYMBOL[b]}</span>" for b in eve_bases[:len(eve_indices)]]), unsafe_allow_html=True)
                
                # Eve measured bits
                st.write("**Eve measured bits:**")
                st.markdown(" ".join([f"<span class='keychip'>{b}</span>" for b in eve_bits[:len(eve_indices)]]), unsafe_allow_html=True)
            else:
                st.info("Eve did not intercept any qubits in this run.")
